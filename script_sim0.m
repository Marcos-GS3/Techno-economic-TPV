...
Ttpv = (20:5:79); % TPV temperature in ºC
...
for j = 1:1:length(LCOrH)
    ...
    % for i=10
        ...
        n_tpv(i) =data_cell(1);
        Pdens_tpv(i) = data_cell(2); % in W/cm2
        CPP_tpv(i) = CPA_tpv*1000/Pdens_tpv(i); % €/kWel TPV cell
        nb_h_year = 365*24;
        teq = CF*nb_h_year;
        ...
        NetSaves(1) = (Cgrid_tot_ref(i) - Cgrid_tot_ye(i,j))/((1+d)^(1));
        Ctot = CAPEX_tot(i)/CRF;
        Ctot_ref = 0;
        ...
        if(isnan(NetSaves(1)))
            ...
            margen = 200;
            for xx=2:n+margen
                NetSaves(xx) = (Cgrid_tot_ref(i) - Cgrid_tot_ye(i,j))*((1+inf_el)^(xx))/((1+d)^(xx));
